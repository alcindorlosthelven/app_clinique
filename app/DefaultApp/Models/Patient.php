...
{
    public $id, $code, $nom, $prenom,$email,$date_naissance, $sexe,$telephone;
    ...
    public function existe($nom, $prenom, $nom_mere, $identite)
    {
        $con = self::connection();
        $req = "SELECT *FROM patient WHERE (nom='" . $nom . "' and prenom='" . $prenom . "' and nom_mere='" . $nom_mere . "') or identite='" . $identite . "' ";
        $rs = $con->query($req);
        if ($data = $rs->fetch()) {
            return true;
        } else {
    ...
    public function addCash(){
        if($this->cashExiste($this->nom,$this->prenom,$this->telephone)){
            return "Client Cash existe";
        }
        return parent::add();
    }
    ...
    public function add()
    {
        if ($this->existe($this->nom, $this->prenom, $this->nom_mere, $this->identite)) {
